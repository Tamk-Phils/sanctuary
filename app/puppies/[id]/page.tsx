...
export default function PuppyPage({ params }: { params: Promise<{ id: string }> }) {
            ...
            try {
                ...
                if (data) {
                    ...
                    if (data.puppy_images && data.puppy_images.length > 0) {
                        setMainImg(data.puppy_images[0]);
                    }
                }
            } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-4">
                    <div className="aspect-[4/3] bg-cream-100 rounded-2xl overflow-hidden border border-cream-200">
                        {mainImg ? (
                            <img src={mainImg} alt={puppy.name} className="w-full h-full object-cover" />
                        ) : (
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            {puppy.puppy_images.map((img, idx) => (
                                <button
                                    ...
                                    className={`aspect-square rounded-xl overflow-hidden border-2 transition-colors ${mainImg === img ? 'border-sand-600' : 'border-transparent hover:border-cream-200'}`}
                                >
                                    <img src={img} alt={`${puppy.name} thumbnail ${idx}`} className="w-full h-full object-cover" />
                                </button>
                            ))}
                        </div>
                    )}
